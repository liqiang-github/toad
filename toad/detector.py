...
def getTopValues(series, top=5, reverse=False):
    ...
    values = [
        template.format(counts[i]) if i < len(counts) else None for i in range(top)
    ]

    return pd.Series(values, index=indexs)


def getDescribe(series, percentiles=[0.25, 0.5, 0.75]):
...
def detect(dataframe):
    ...
    for name, series in dataframe.items():
        ...
        if isNumeric(series):
            desc = getDescribe(series, percentiles=[0.01, 0.1, 0.5, 0.75, 0.9, 0.99])
            details = desc.tolist()
        else:
