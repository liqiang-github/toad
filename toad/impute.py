...
class Imputer(IterativeImputer):
    ...
    def _impute_one_feature(
        self, X_filled, mask_missing_values, feat_idx, neighbor_feat_idx, **kwargs
    ):

        return super()._impute_one_feature(
            X_filled, mask_missing_values, feat_idx, neighbor_feat_idx, **kwargs
        )
    ...
    def _fit_encode(self, X, mask):
        ...
        for col in category_data:
            unique, X[col].loc[~mask[col]] = np.unique(
                X[col][~mask[col]], return_inverse=True
            )
