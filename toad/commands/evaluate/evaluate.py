...
def rename_columns(index_list, bins, is_float=False):
    ...
    for i in range(len(list)):
        if i == 0:
            bins_list.append(
                left
                + str(float("%.2f" % bins[list[0]]) if is_float else int(bins[list[0]]))
                + ")"
            )
        elif i == len(list) - 1:
            bins_list.append(
                "["
                + str(
                    float("%.2f" % bins[list[len(list) - 1] - 1])
                    if is_float
                    else int(bins[list[len(list) - 1] - 1])
                )
                + ",inf)"
            )
        else:
            bins_list.append(
                "["
                + str(
                    float("%.2f" % bins[list[i - 1]])
                    if is_float
                    else int(bins[list[i - 1]])
                )
                + ","
                + str(float("%.2f" % bins[list[i]]) if is_float else int(bins[list[i]]))
                + ")"
            )
    return bins_list
...
def self_bin_dispersed(X, Y):
    ...
    for name, group in d2:
        ...
        d3 = d3.append(
            {"bins": name, "bad": bad_count, "fre": total, "bad_rate": bad_rate},
            ignore_index=True,
        )
...
def draw_data(workbook, worksheet, data, **kwargs):
    ...
    haed_format = workbook.add_format(
        {"bold": True, "valign": "middle", "align": "center", "border": 1}
    )
    data_format = workbook.add_format(
        {"valign": "middle", "align": "center", "border": 1}
    )
    ...
    worksheet.merge_range(
        ...
        + str(start_index + 1)
        + ":"
        + column_to_name(len(columns_list) - 1).upper()
        + str(start_index + 1),
        title,
        merge_format,
    )
    ...
    for j in range(len(columns_list)):
        worksheet.write_column(
            column_to_name(j) + str(start_index + 3), data[columns_list[j]], data_format
        )
...
def merger_data(data, var, unique_num, is_merge_high=True):
    if is_numeric_dtype(data[var]) and data[var].nunique() > unique_num:
        ...
        if is_merge_high:
            merge_high_data = toad.utils.clip(data_nomiss[var], quantile=(None, 0.99))
        data_index, bins = toad.merge(
            merge_high_data, method="step", return_splits=True, n_bins=unique_num
        )
...
def crosstab_data(columns_var, row_var, data, unique_num, *args):
    columns_data, columns_target, columns_bins = merger_data(
        data, columns_var, unique_num, args[0]
    )
...
def all_var_pic(high_iv_var_list, data, workbook, worksheet, start_index, unique_num):
    for i in range(len(high_iv_var_list)):
        ...
        start_index = save_bins_and_chart(
            worksheet, workbook, result, var_name, start_index
        )
    return start_index
...
def save_overdue_crosstab(data, var_list, workbook, worksheet, start_index, unique_num):
    for i in range(len(var_list)):
        var = var_list[i]
        result = crosstab_data(
            var, "overdue_days", data, unique_num, True, False, True, False
        )
        columns = ["overdue_days"] + result.columns.tolist()
        result["overdue_days"] = result.index
        start_index = draw_data(
            workbook,
            worksheet,
            result,
            start_index=start_index,
...
# 按照变量分组循环建模，并计算ks
def compute_ks(
    data_train, data_test, quality, self_var_list, self_var_ks, result, target="target"
):
    ...
    for temp in test_var_list:
        ...
        model, param = param_opt(
            data_train[input_var + self_var_list], data_train[target]
        )
        train_ks = get_ks(
            model, data_train[input_var + self_var_list], data_train[target]
        )
        ...
        result = result.append(
            {"train_ks": train_ks, "test_ks": test_ks, "var": str(input_var)},
            ignore_index=True,
        )
    return result
...
def test_score(p, x_train, y_train):
    ...
    score_mean = cross_val_score(
        model_ada, x_train, y_train, cv=10, scoring=scorer
    ).mean()
    return score_mean
...
def findMax(start, end, results, train_x, train_y):
    ...
    if results[need_test] > max(results[start], results[end]) or results[
        need_test
    ] < min(results[start], results[end]):
...
def evaluate(
    ...
    if self_data is not None:
        ...
        result = result.append(
            {"train_ks": train_ks, "test_ks": test_ks, "var": "[]"}, ignore_index=True
        )
        result = compute_ks(
            data_train,
            data_test,
